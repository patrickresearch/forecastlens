...
def _forecast(quantiles, n=4, point=None):
    timestamps = pd.date_range("2024-01-01", periods=n, freq="D")
    return ForecastResult(timestamps=timestamps, freq="D", quantiles=quantiles, point=point)
...
def test_hand_computed_overall_and_per_regime_metrics():
    ...
    # `point` is set equal to y_true exactly, so median() (falling back to
    # point, since no 0.5 quantile is given) makes mae/rmse/wape/accuracy
    # trivial (0/0/0/1) without disturbing the quantile levels used above.
    y_true = np.array([10.0, 10.0, 20.0, 20.0])
    forecast = _forecast(
        {0.25: [8.0, 8.0, 18.0, 18.0], 0.75: [12.0, 12.0, 22.0, 22.0]},
        point=[10.0, 10.0, 20.0, 20.0],
    )
    ...
    assert report.overall_wape == pytest.approx(0.0)
    ...
    assert regime0.mae == pytest.approx(0.0)
    assert regime0.rmse == pytest.approx(0.0)
    assert regime0.wape == pytest.approx(0.0)
    assert regime0.accuracy == pytest.approx(1.0)
    ...
    assert regime1.mae == pytest.approx(0.0)
    assert regime1.rmse == pytest.approx(0.0)
    assert regime1.wape == pytest.approx(0.0)
    assert regime1.accuracy == pytest.approx(1.0)


def test_mae_rmse_wape_differ_from_zero_when_median_is_off():
    # forecast.median() prefers a 0.5 quantile over `point` when both exist,
    # so omit 0.5 here to actually exercise `point` -- deliberately off by a
    # constant +2 from y_true at every point.
    y_true = np.array([10.0, 20.0])
    forecast = _forecast({0.25: [9.0, 19.0], 0.75: [11.0, 21.0]}, n=2, point=[12.0, 22.0])
    detector = _FakeDetector(regime_labels=[0, 0])

    report = RegimeAwareEvaluator(detector).evaluate(forecast, y_true)

    # median() falls back to `point` = [12, 22]; errors are +2 at both points.
    assert report.overall_mae == pytest.approx(2.0)
    assert report.overall_rmse == pytest.approx(2.0)
    assert report.overall_wape == pytest.approx(4.0 / 30.0)  # sum(|diff|)=4, sum(|y_true|)=30
    assert report.overall_accuracy == pytest.approx(1.0 - 4.0 / 30.0)
